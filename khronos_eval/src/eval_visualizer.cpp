...
void EvalVisualizer::drawBboxes(const VisData& data) {
  ...
  if (!bbox_gt_marker_) {
    ...
    for (const auto& [_, object] : data.dsg_objects) {
      ...
      auto bbox_marker = setBoundingBox(object.bbox, color, header_);
      for (auto& point : bbox_marker.points) {
        point.x += object.bbox.world_P_center.x();
        point.y += object.bbox.world_P_center.y();
        point.z += object.bbox.world_P_center.z();
    ...
    for (const auto& [_, object] : data.gt_objects) {
      ...
      auto bbox_marker = setBoundingBox(bbox, color, header_);
      for (auto& point : bbox_marker.points) {
        point.x += bbox.world_P_center.x();
        point.y += bbox.world_P_center.y();
        point.z += bbox.world_P_center.z();
        marker.points.push_back(point);
      }
      marker.colors.insert(marker.colors.end(), bbox_marker.points.size(), setColor(color));
    }
